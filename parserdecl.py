...
class SymbolTableException(Exception):
    def __init__(self, lineno: int, ID: str) -> None:
        message = (
            "Symbol table error on line: " + str(lineno) + "\nUndeclared ID: " + str(ID)
        )
...
class SymbolTable:
    ...
    def insert(
        ...
        id_type: IDType,
        data_type: Type,
        nng: Optional[NewNameGenerator] = None,
    ) -> None:
...
class Parser:
    ...
    def parse_arg(self) -> Tuple[str, str]:
        ...
        if token_id == Token.FLOAT:
            ...
            raise ParserException(
                self.scanner.get_lineno(), self.to_match, [Token.INT, Token.FLOAT]
            )
        self.eat(Token.AMP)
    ...
    def parse_if_else_statement(self) -> List[str]:
        ...
        compare_ins = [
            "%s = int2vr(0);" % (zero_vr),
            "beq(%s, %s, %s);" % (expr_ast.vr, zero_vr, else_label),
        ]
    ...
    def parse_for_statement(self) -> List[str]:
        ...
        # NEW: Identify iteration variable
        try:
            logging.debug(
                "Original assignment program: %s" % original_assignment_program
            )
            iterationVar = re.match(
                r"(\w+) = vr\d+;", original_assignment_program[1]
            ).group(1)
            logging.debug("Iteration variable: %s" % iterationVar)
            iterationConstInt = re.match(
                r"int2vr\((\d+)\);", original_assignment_program[0].split(" ")[2]
            ).group(1)
            isConstInt = iterationConstInt != None
            logging.debug("Is const int: %s" % isConstInt)
        except:
            iterationVar = None
            iterationConstInt = None
            isConstInt = False
        ...
        # NEW: Check if condition expr is in right form
        isCondExprLTWithIterationOnLHSAndIntegerConstantOnRHS = (
            type(expr_ast) == ASTLtNode
            and type(expr_ast.l_child) == ASTVarIDNode
            and expr_ast.l_child.vr == iterationVar
            and type(expr_ast.r_child) == ASTNumNode
        )

        endCondConst = expr_ast.r_child.value

        logging.debug(
            "Is condition expression LT with iteration variable on LHS and integer constant on RHS: %s"
            % isCondExprLTWithIterationOnLHSAndIntegerConstantOnRHS
        )
        ...
        # NEW: Identify if loop end assignment is incrementing the iteration variable
        logging.debug("Loop end assignment program: %s" % loop_end_assignment_program)
        tempVar = re.match(
            r"(\w+) = addi\((\w+),(\w+)\);", loop_end_assignment_program[1]
        ).group(1)
        tempVar2 = re.match(
            r"(\w+) = int2vr\((\d+)\);", loop_end_assignment_program[0]
        ).group(1)
        logging.debug("Temp var: %s" % tempVar)
        logging.debug("Temp var2: %s" % tempVar2)
        correctIter = [
            f"{tempVar2} = int2vr(1);",
            f"{tempVar} = addi({iterationVar},{tempVar2});",
            f"{iterationVar} = {tempVar};",
        ]
        logging.debug("Correct iter: %s" % correctIter)
        isIncrementingIterationVariable = loop_end_assignment_program == correctIter
        logging.debug(
            "Is incrementing iteration variable: %s" % isIncrementingIterationVariable
        )

        self.eat(Token.RPAR)
        loop_program = self.parse_statement()

        # NEW: Check if loop body has control flow
        logging.debug("Loop program: %s" % loop_program)
        noControlFlow = True
        for line in loop_program:
            if line.startswith("branch") or line.startswith("beq"):
                noControlFlow = False
                break
        logging.debug("No control flow: %s" % noControlFlow)
        # NEW: Updates iteration variable
        noUpdate = True
        for line in loop_program:
            if line.startswith(f"{iterationVar} ="):
                noUpdate = False
                break
        logging.debug("No update: %s" % noUpdate)

        # Check unrolling factor divides loop iterations evenly
        # Parse CLI argument
        import argparse
        ...
        if unroll_factor >= 1:
            cleanDiv = (int(endCondConst) - int(iterationConstInt)) % unroll_factor == 0
        else:
            cleanDiv = False
        logging.debug("Clean div: %s" % cleanDiv)

        logging.debug("Unroll factor: %s" % unroll_factor)

        loop_start_label = self.nlg.mk_new_label()
        end_label = self.nlg.mk_new_label()
        iter_vr = self.vra.mk_new_vr()
        end_vr = self.vra.mk_new_vr()

        if (
            isConstInt
            and isCondExprLTWithIterationOnLHSAndIntegerConstantOnRHS
            and isIncrementingIterationVariable
            and noControlFlow
            and noUpdate
            and cleanDiv
        ):
            logging.debug("Unrolling loop")
            return (
                [
                    f"{iter_vr} = int2vr(1);",
                    f"{end_vr} = int2vr({int(endCondConst) - int(iterationConstInt)});",
                    f"{iterationVar} = int2vr({iterationConstInt});",
                    f"beq({iterationVar}, {end_vr}, {end_label});",
                    f"{loop_start_label}:",
                ]
                + (loop_program + loop_end_assignment_program) * unroll_factor
                + [
                    f"bneq({iterationVar}, {end_vr}, {loop_start_label});",
                    f"{end_label}:",
                ]
            )

        else:
            logging.debug("Not unrolling loop")
            compare_ins = [
                "%s = int2vr(0);" % (end_vr),
                "beq(%s, %s, %s);" % (expr_ast.vr, end_vr, end_label),
            ]  # Branch out if expression == 0
            branch_ins = [
                "branch(%s);" % (loop_start_label)
            ]  # Instruction to branch back to the start of the loop (right before evaluating the expression again)

            return (
                original_assignment_program
                + ["%s:" % (loop_start_label)]
                + expr_program
                + compare_ins
                + loop_program
                + loop_end_assignment_program
                + branch_ins
                + ["%s:" % (end_label)]
            )
    ...
    def parse_expr2(self, lhs_node: ASTNode) -> ASTNode:
        ...
        raise ParserException(
            self.scanner.get_lineno(), self.to_match, [Token.EQ, Token.SEMI, Token.RPAR]
        )
    ...
    def parse_unit(self) -> ASTNode:
        ...
        raise ParserException(
            self.scanner.get_lineno(), self.to_match, [Token.NUM, Token.ID, Token.LPAR]
        )
...
def type_inference(node: ASTNode) -> Type:
    ...
    if is_leaf_node(node):
        ...
        if type(node) in [ASTPlusNode, ASTMinusNode, ASTMultNode, ASTDivNode]:
            ...
            if (
                node.l_child.node_type == Type.FLOAT
                or node.r_child.node_type == Type.FLOAT
            ):
